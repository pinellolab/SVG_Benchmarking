...
source("/data/pinello/PROJECTS/2023_03_SVGBenchmarking/workflow/scripts/utils/BOOST-GP/functions.R");
Rcpp::sourceCpp("/data/pinello/PROJECTS/2023_03_SVGBenchmarking/workflow/scripts/utils/BOOST-GP/boost.cpp");
# =========================================================================================
...
boost.gp <- function(Y, loc, iter = 1000, burn = 500, size.factor = NA, init.b.sigma = NA, init.h = 1, 
    ...
    for (i in 1:chain) {
        ...
        res <- boost(Y = as.matrix(Y), dist = dist.mat$dist, nei = dist.mat$nei, 
                     ...
                     init_b_sigma = init.b.sigma, init_h = init.h, 
                     update_prop = update_prop) # See the definition of each argument in the header                               
